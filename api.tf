locals {
  api_chart_path = "${path.module}/charts/api"
}

resource "helm_release" "api" {
  name       = "api"
  chart      = local.api_chart_path
  reset_values = true

  set {
    name  = "chart-hash"
    value = sha1(join("", [for f in fileset(local.api_chart_path, "**/*.yaml"): filesha1("${local.api_chart_path}/${f}")]))
  }
